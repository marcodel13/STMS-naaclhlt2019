...
also find systematic deviations. A small, but consistent group is that of \textit{false negatives}: 
...
developed throughout the whole text produced by an author. For instance, besides the {\em `shovel'} example mentioned in Section~\ref{sec:sources}, we find the word `pharaoh' as the nickname of an Egyptian player who
...
A larger group of problematic cases is that of \textit{false positives}, that
...
{\em `entourage'}, for the entourage a particular star of the team; {\em `independence'} for the
