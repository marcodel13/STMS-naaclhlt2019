...
contextual variability as follows:\footnote{\marco{While there can be other potential ways to measure context variability, we expect them to yield the same picture.}} for a target word, we create a
...
pairwise cosine distance between context vectors.\footnote{\marco{We experimented with a different window size and obtained the same pattern.}} 
We find that
