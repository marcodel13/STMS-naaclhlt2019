...
% \footnote{As expected, all the words that do not increase in frequency have very low shift index and low cosine distance, meaning that the model reliably identify them as cases of \textit{no} meaning shift. \raq{I moved this to footnote, but I'm actually not convinced by the text: the points to make I guess are: the one with no frequency change have low shift index, this sort of validates our selection method since we had assumed that it would be too difficult to randomly find examples of shift. The other point about the model I'm not sure it actually makes sense here...}}
% gbt moved to previous section
...
Among the words that undergo a strong shift are, for example, `highlighter', which occurs in sentences like \textit{`we are playing with the \textbf{highlighter} today'}, a metonymy used to talk about a kit in a colour similar to that of a highlighter, or `lean', in \textit{`I hope a \textbf{lean} comes soon!'}, due to players typically leaning on a Liverpool symbol when posing for a photo right after signing for the club. Particularly explanatory is the `F5' example shown (in chronological order) in
...
A much smaller, but consistent group is that of \textit{false negatives}, 
