...
Therefore, all models (including ours) spot semantic shift as a change in
the word representation in different time periods.
% computation of time-related distributional representations for words
% in the vocabulary, and the sequential comparison of such
% representations in order to detect a drop in
% self-similarity, % usually
% interpreted as a shift in meaning. 
% gbt: the previous explanation was a bit cryptic imo (time-related
% representations, self-similarity, ...)
This includes classic
distributional representations obtained with count-based models, topic
modelling or Latent Semantic Analysis
\cite{sagi2011tracing,jatowt2014framework,wijaya2011understanding,rohrdantz2011towards,gulordava2011distributional,xu2015computational}
as well more recent, neural-network based word embeddings
% as more recent word embeddings computed using the skip-gram
% algorithm of \newcite{mikolov2013distributed}
\cite{kulkarni2015statistically,azarbonyad2017words,hamilton2016diachronic,kim2014temporal,del2016tracing,phillips2017intrinsic,szymanski2017temporal}.
Note, however, that distributional representations from different time periods cannot be
directly compared. 
%From the methods to enable comparison that have
%been proposed in the literature just cited, 
...
Evaluation of semantic shift is difficult, due of the lack of
annotated datasets \cite{frermann2016bayesian}. For this reason, even
for long-term shift evaluation is usually performed by manually
inspecting the $n$ words whose representation changes the most
according to the
model~\cite{hamilton2016diachronic,del2016tracing,kim2014temporal}.
